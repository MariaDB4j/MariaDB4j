...
public class DB {
	...
	/**
	 * This factory method is the mechanism for constructing a new embedded
	 * database for use. This method automatically installs the database and
	 * prepares it for use.
	 * 
	 * @param config
	 ...
	 * This factory method is the mechanism for constructing a new embedded
	 * database for use. This method automatically installs the database and
	 * prepares it for use with default configuration, allowing only for
	 * specifying port.
	 * 
	 * @param port
	 *            the port to start the embedded database on
	 * @return a new DB instance
	 * @throws ManagedProcessException
	 *             if something fatal went wrong
	 */
	...
	/**
	 * Adds a shutdown hook to ensure that when the JVM exits, the database is
	 * stopped, and any temporary data directories are cleaned up.
	 */
		...
		Runtime.getRuntime().addShutdownHook(new Thread(threadName) {
			...
			public void run() {
				...
				if (dataDir.exists() && (configuration.isDeletingTemporaryBaseAndDataDirsOnShutdown()
					...
					logger.info(
							"cleanupOnExit() ShutdownHook quietly deleting temporary DB data directory: " + dataDir);
					FileUtils.deleteQuietly(dataDir);
				}
				if (baseDir.exists() && (configuration.isDeletingTemporaryBaseAndDataDirsOnShutdown()
						&& Util.isTemporaryDirectory(dataDir.getAbsolutePath()))) {
					logger.info(
							"cleanupOnExit() ShutdownHook quietly deleting temporary DB base directory: " + baseDir);
	...
	protected ManagedProcess dump(File outputFile, List<String> dbNamesToDump, boolean compactDump, boolean lockTables,
			boolean asXml, String user, String password) throws ManagedProcessException, IOException {
	...
	/**
	 * Config Socket as absolute path. By default this is the case because
	 * DBConfigurationBuilder creates the socket in /tmp, but if a user uses
	 * setSocket() he may give a relative location, so we double check.
	 * 
	...
	ManagedProcess installPreparation() throws ManagedProcessException, IOException {
		...
		if (!installDbCmdFile.exists())
			throw new ManagedProcessException("mysql_install_db was not found, neither in bin/ nor in scripts/ under "
	...
	/**
	 * If the data directory specified in the configuration is a temporary
	 * directory, this deletes any previous version. It also makes sure that the
	 * directory exists.
	 * 
	 * @throws ManagedProcessException
	 *             if something fatal went wrong
	 ...
	 * it via the mysql command line tool.
	 * 
	 * @param resource
	 *            the path to a resource on the classpath to source
	 * @param username
	 *            the username used to login to the database
	 * @param password
	 *            the password used to login to the database
	 * @param dbName
	 *            the name of the database (schema) to source into
	 * @throws ManagedProcessException
	 *             if something fatal went wrong
	 */
	...
	/**
	 * Starts up the database, using the data directory and port specified in
	 * the configuration.
	 * 
	 * @throws ManagedProcessException
	 *             if something fatal went wrong
	...
	// (even though with --lock-tables one could not run two dumps concurrently
	// anyway)
	// because in theory this could cause a long-running dump to deadlock an
	// application
	// wanting to stop() a DB. Let it thus be a caller's responsibility to not
	// dump
	// concurrently (and if she does, it just fails, which is much better than
	// an
	// unexpected deadlock).
	...
	/**
	 * Based on the current OS, unpacks the appropriate version of MariaDB to
	 * the file system based on the configuration.
	 */
